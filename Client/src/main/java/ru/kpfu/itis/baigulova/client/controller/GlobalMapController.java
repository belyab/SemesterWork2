...
import ru.kpfu.itis.baigulova.client.model.Question;
...
public class GlobalMapController implements Initializable {
    ...
    public void initialize(URL location, ResourceBundle resources) {
        new Thread(() -> {
            Client socketService = Client.getInstance();
            try {
    ...
    private void updateUI() throws IOException {
        ...
        Platform.runLater(() -> {
            ...
            if (isYourStage)
                ...
                moveLabel.setText("Ход соперника");
        });
    }
    ...
    private void stateOfMove(int stateMove) {
        ...
        if (stateMove == 1) {
            ...
            alert.setHeaderText("У вас больше завоеванных территорий чем у соперника");
        }
        else if (stateMove==3){
            alert.setTitle("Вы проиграли!");
            alert.setHeaderText("У вас меньше завоеванных территорий чем у соперника");
        }
        else if (stateMove==4){
            alert.setTitle("Ничья!");
            alert.setHeaderText("У вас одинаковое количество завоеванных территорий");
        }
        alert.showAndWait();
        if (stateMove==2 || stateMove==3 || stateMove==4){
            ...
            client.disconnect();
            try {
                WindowManager.renderLoginWindow(MainClient.primaryStage);
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
                ...
                if (isYourStage) {
                    Platform.runLater(() -> {
                        try {
                            ...
                            client.sendMessage("/attack " + finalK);
                            String msg = client.waitQuestion();
                            getQuestion(msg);
                        } catch (IOException e) {
                            e.printStackTrace();
                        }
    ...
    private void setGameQuestion(Question question) {
        ...
        textLabel.setText(question.getQuestionText());
    ...
    private void setAnswerListener() {
        ...
        for (Button answerButton : answersButtons) {
            ...
            answerButton.addEventHandler(MouseEvent.MOUSE_CLICKED, event -> {
                ...
                try {
                    client.sendMessage("/answer " + finalIndex);
                    waitAnswer();
                    Platform.runLater(() -> {
